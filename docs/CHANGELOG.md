...
- **SDK 兼容宿主内置 so（完整引入场景）**：SDK 的可选组件对宿主是 `compileOnly`，宿主用 `implementation` 引入时 so 随 APK 分发。原判定把「so 就位」等同于「下载目录里有文件」，导致宿主内置 so 后被误判为未安装——内核静默回退系统播放器、面板还引导用户下载一份 APK 里已有的 so。新增 `InstallSource(NONE/DOWNLOADED/BUNDLED)`，判定按 APK 优先，并拆出 `canRemove`（内置的 so 删不掉，面板显示「内置」而非「删除」）
- **扩展包管理入口移到设置面板**：该面板管的是种子/语音/文字识别/翻译/播放内核，属应用级能力，原先挂在字幕设置面板底部归类不当。现位于全屏设置弹窗的「播放核心」下方（主界面设置面板保留入口），并补齐四个内核的下载行
- **完整版 APK**：`-PorangeFullNativeLibs=true` 可构建 so 全部内置的版本（宿主装完即用，无需下载），与按需下载版同时发布，便于对比

### 🐛 修复

- **扩展包按钮文字只显示上半截**：宿主是 Material3 主题，`<Button>` 被替换成 `MaterialButton`（自带 6dp 上下 inset 与较大垂直 padding），压到 32dp 后 12sp 文字装不下。改用 `TextView` + `gravity=center`
- **下载失败后「重试」点了没反应**：只改了文案没重新挂监听。另 `setEnabled(false)` 对 `TextView` 无视觉反馈且仍收点击，改为 `setAlpha` + 清监听
- **扩展包状态与实际不一致**：`isInstalled()` 在 `NativeLibManager` 未初始化时返回 false，而 `download()` 会惰性补初始化，导致面板把已下载的组件显示成「未安装」、点下载却瞬间完成。现面板渲染前先确保初始化，两处判定共用同一状态

### 🔧 优化

- **扩展包面板改紧凑单行卡片**：原每项占约 190dp（标题+状态+大按钮+分隔线），8 项要滚两屏多；现每行 56dp、按「功能组件/播放内核」分卡片，一屏可览

### 🏗️ 构建与发布

- **CI 装 NDK**：AGP 的 `stripReleaseDebugSymbols` 在缺 NDK 时**静默跳过**，so 带调试符号进 APK——v1.5.2 的 arm64 包因此 140MB（本地同代码 125MB）。补装 NDK 并加体积守卫，防止此类静默降级再发生
- **APK workflow 加 variant 维度**：tag 触发时自动构建 6 个 APK（3 ABI × slim/full），不再需要本机构建后手动上传
- **组件 zip 改为本地构建**：`zipfile.ZIP_DEFLATED` 的字节流依赖 zlib 版本，跨环境 sha256 不可复现（本机 1.3.1 vs runner 3.12）。workflow 改为只校验已上传资产与 Java 常量一致
...
- **播放内核也支持按需下载**：IJK / 阿里云 / MPV / FFmpeg 合并组件不再随安装包分发，首次使用内核时引导下载（断点续传 + 多镜像）。四个内核的可用性收敛到 `PlayerEngineAvailability`，未下载的内核不再「显示可用但静默回退到系统内核」
- **扩展包管理面板**：字幕设置面板底部新增入口，可查看/下载四个功能组件与四个播放内核

### 🐛 修复

- **点击下载内核提示下载失败**：v1.5.2 的 Release 资产只有四个功能组件 zip，没有内核 zip，`RELEASE_TAG` 拼出的地址全部 404
- **下载 ffmpeg 组件后本次运行仍不可用**：`FFmpegKit` 首次加载失败会进入 stub 模式并锁定，新增 `resetForRetry()`，下载完成后重置，免重启即可用
...
- **嗅探无面板、结果显示黑屏**：`initSniffingView()` 此前只在 `setVideoController()` 里调用，而 v1.5.x 控制器去重修复后宿主复用 SDK 自动创建的控制器、不再走该路径，导致嗅探面板从不创建。现改在播放器组件初始化末尾调用并加幂等守卫

### ✨ 新增

- **嗅探支持 MSE/blob 播放器站点（hls.js / dash.js 类）**：注入 JS 钩住 `XMLHttpRequest` 与 `fetch` 并扫描 `<video>` 源，把无扩展名的媒体 URL 经桥接回传给既有异步验证链路，补齐纯网络拦截看不到的盲区
...
- **关闭「边看边识别」后已识别字幕仍在屏幕滚动**：关闭时除摘掉悬浮环外，一并回收本会话注入的识别字幕（此前旧字幕留在屏上且不再更新，用户不知来源）
- **模型下载弹窗是假进度**：MLKit 翻译模型下载弹窗改为真实字节进度——轮询「模型目录增长 + 系统 DownloadManager 在途字节」，第 0 秒起即随下载推进（此前只算模型目录，而 MLKit 下载完成前字节在系统下载缓存里，界面会长时间停在 0% 像卡死）。AI 设置「预装本地翻译模型」与 OCR 翻译共用该进度
- **非中日韩语种本地翻译静默失效**：`MlKitTranslationEngine` 靠「语言码转大写」猜 MLKit 常量名，只对 zh/en/ja/ko 成立，法/德/西/俄/阿/泰/越 拼出不存在的常量（FR 而非 FRENCH）→ 初始化失败。选这些目标语言时预装与本地兜底全部跳过，现按全称常量映射并加单测锁定

### 🔒 兼容性

- **API Key 输入框不再被判定为密码字段**（OPPO 等机型会因此强制切系统安全键盘，第三方输入法与粘贴不可用）：移除 `inputType="textPassword"` 与 `PasswordTransformationMethod`，保持普通文本、明文显示

---

## [1.5.0] - 2026-09-12

### 🎙️ AI 语音字幕（sherpa-onnx SenseVoice，新增可选模块 orangeplayer-sherpa）

- **离线语音识别**：新增 `orangeplayer-sherpa` 模块，SenseVoice 模型（中/英/日/韩/粤，约 228MB）应用内下载——多源（hf-mirror 优先）+ 断点续传 + Content-Length 校验，截断文件自动重下
- **完整版生成**：视频直出字幕（抽音轨→重采样→分段识别→srt 注入），HLS 自动选变体下载后识别
- **边看边识别（渐进）**：本地 / 已缓存 mp4 与 HLS 按播放进度逐块识别，看一段出一段字幕；HLS 分片复用播放器 media3 缓存，零额外流量；长段按 2~3s 切分时间轴
- **悬浮进度环**：可拖动，拖到边缘收成 10% 纯色把手（点按/拖动恢复），自动贴边已取消（5 秒自动收起与「边看边识别」语义冲突）
- **启动清扫** `asr_work` 孤儿临时文件（崩溃残留的 `_raw.pcm` 可达 70MB+）

### 🌐 识别后自动翻译（ASR → 翻译）

- **双链路**：配置 API Key 走 AI 翻译；未配置自动降级本地 MLKit 离线模型（按「语言↔英语」存储，英语中转，已装语言不重复下载）
- **工程化**：背压合并待译区间（AI 超时故障下队列不再只增不减）、AI 连续失败锁存本地、术语表落盘缓存、断点续译、真实下标回写（避免列表替换后译文错位）

### ⚙️ AI 设置增强

- **目标语言**改下拉选择，范围与本地兜底可译语言对齐；**源语言**选择持久化
- **预装本地翻译模型**：下拉标注「已装/未装」，按语言对预下载（OCR 翻译与语音识别共用同一份模型）
- **获取模型**：调 OpenAI 兼容 `GET /models` 拉取服务端模型列表，单选回填，免手输模型名出错
- **改目标语言即时重译**：会话中途切换目标语言会重译已识别字幕（此前需重开开关或重播视频）
- 设置面板改为与字幕设置同宽的右侧面板（原全屏下铺满整屏）；保存提示明确「已保存」并分别说明两条链路可用性

### 📺 HLS 与字幕

- **本地 HLS 分片代理**：ijk 等非 Exo 内核复用磁盘缓存，零额外流量
- 去广告清洗并入 `HlsProxyServer` 重写管线（同一份 playlist 单次抓取）
- 修复换视频残留旧字幕、空字幕半透明背景条永久挂屏、普通字幕循环永不显示（alpha 恒为 0）、显示去尾部标点
- 修复 mpv 内核 HLS 代理 URL 被 `setDataSource` 原始地址覆盖

### 🗑️ 移除

- **移除 Vosk 语音识别路径**（`AudioPlaybackCapture` + MediaProjection）：能力已被 sherpa 渐进识别完全覆盖，且旧路径需录屏授权、只能识别正在播放的内容。删除 10 个类、3 个布局、MANIFEST 服务与权限及 vosk 依赖，**APK 体积 160.7MB → 148.0MB**
