...
- **扩展包管理入口移到设置面板**：该面板管的是种子/语音/文字识别/翻译/播放内核，属应用级能力，原先挂在字幕设置面板底部归类不当。现位于「设置 → 高级功能 → 扩展包管理」，并补齐四个内核的下载行
...
- **下载 ffmpeg 组件后本次运行仍不可用**：`FFmpegKit` 首次加载失败会进入 stub 模式并锁定，下载完成后新增 `resetForRetry()` 重置，免重启即可用

### 🔧 优化

- 引擎按钮的「待下载」不再用文本 `Ali↓` 表达（破坏等宽对齐、视觉粗糙），改为未就绪时文字变暗，点击仍走下载引导
